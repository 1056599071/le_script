...
hive -e "select ${yesterday}, count(1) pv, count(distinct letv_cookie) uv, props from dwb.dwb_megatron_action_hour where dt = '${yesterday}' and country = 'cn' and app_name = 'vipCashier' and (props like 'ch=zhifu%') group by props" >> ./data/query_hook_data.${yesterday}
