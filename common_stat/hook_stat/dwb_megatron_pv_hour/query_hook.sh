...
#pcVip="(cur_url like '%zhifu.le.com/tobuy/pro%' or cur_url like '%zhifu.le.com/tobuy/regular%')"
#mVip="(cur_url like '%zhifu.le.com/mz/tobuy/pro%' or cur_url like '%zhifu.le.com/mz/tobuy/regular%')"
#20170302切换域名之后
pcVip="cur_url like '%ibuy.le.com/v2/buy/package.html%'"
mVip="cur_url like '%ibuy.le.com/v2/buy/package.html%'"
...
where dt = '${yesterday}' and prod = 'vipCashier' and country = 'cn' and platform = '0' and $pcVip and filter_url(cur_url) <> 'NULL'
...
where dt = '${yesterday}' and prod = 'vipCashier' and country = 'cn' and platform = '0' and $pcVip and filter_url(cur_url) <> 'NULL') t1
inner join
...
select filter_url(cur_url) as ref, count(1) as pv, count(distinct session_id) as uv from dwb.dwb_megatron_pv_hour
where dt = '${yesterday}' and prod = 'vipCashier' and country = 'cn' and platform in ('1', '2') and $mVip and filter_url(cur_url) <> 'NULL'
group by filter_url(cur_url)
) tt1 left join (
select t1.ref, count(1) as pay_pv, count(distinct t2.userid) as pay_uv, 
count(case when t2.neworxufei = '0' then 1 end) as new_order_count, count(case when t2.neworxufei = '1' then 1 end) as xufei_order_count, 
coalesce(sum(case when t2.neworxufei = '0' then t2.money end), 0) as new_income, coalesce(sum(case when t2.neworxufei = '1' then t2.money end), 0) as xufei_income from 
(select distinct uid, filter_url(cur_url) as ref from dwb.dwb_megatron_pv_hour
where dt = '${yesterday}' and prod = 'vipCashier' and country = 'cn' and platform in ('1', '2') and $mVip and filter_url(cur_url) <> 'NULL') t1
inner join
