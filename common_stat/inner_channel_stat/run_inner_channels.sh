...
hive -e "add jar /home/zhaochunlong/boss_stat/common_stat/inner_channel_stat/boss_filter_url.jar;
...
(select userid,status,neworxufei,dt from dm_boss.t_new_order_4_data where dt>='${yesterday}' and dt<'${today}' and orderpaytype!='-1' and status='1' and terminal='112' and ordertype not in (0,1,1001))t1
join
...
left outer join
(select t2.uid,t2.letv_cookie,t2.cur_url_ref,t1.neworxufei,t2.dt from
(select userid,status,neworxufei,dt from dm_boss.t_new_order_4_data where dt>='${yesterday}' and dt<'${today}' and orderpaytype!='-1' and status='1' and terminal='113' and ordertype not in (0,1,1001))t1
join
