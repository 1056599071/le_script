...
on(t1.userid=t2.uid)
group by t1.can" > ./data/membership_valid_30.${ssdate}
...
right JOIN
(select uid,id from data_sum.sum_user_pv_day where dt='${ssdate}' and product='tv' and ilu='0')t2
on(t1.userid=t2.uid) 
where t1.userid is null" > ./data/login_invalid.${ssdate}
